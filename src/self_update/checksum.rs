...
pub fn sha256_file(path: &Path) -> AllpResult<String> {
...
mod tests {
    ...
    fn checksum_mismatch_is_rejected() {
        let path = std::env::temp_dir().join(format!(
            "allp-checksum-mismatch-{}-{:?}",
            std::process::id(),
            std::thread::current().id()
        ));
        std::fs::write(&path, b"verified payload").expect("fixture should be written");
        let error = verify_sha256(&path, &"0".repeat(64)).expect_err("wrong checksum must fail");
