...
The Cargo package version remains strict three-part SemVer (`0.3.5`). A compiled `AllpBuildIdentity` adds a monotonically increasing build revision, commit, build ID, timestamp, target, channel, and official-provenance bit. The requested build therefore displays `0.3.5.1` without setting an invalid four-part Cargo version. `allp --version --verbose` exposes the complete identity. Local builds display the requested revision but are marked `development` and non-official; only CI can set official provenance.

Continuous comparison orders base SemVer first and build revision second. Thus `0.3.5.2` updates `0.3.5.1`, `0.3.6.1` updates `0.3.5.99`, and `0.3.5.200` cannot downgrade `0.3.6.1`. Equal revision with different commits is an integrity error. A rerun with the same source commit is normally not forced even if its workflow identity changes.
